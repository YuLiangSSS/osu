...
{
    /// <summary>
    /// A standalone skin component that reproduces the behaviour of the legacy health display
    /// but sources its value from the O2 pill bindable.
    /// This merges the relevant parts of HealthDisplay and LegacyHealthDisplay into a single file so
    /// it can be used independently from the ruleset health processor.
    /// </summary>
    public partial class O2PillBarLegacy : CompositeDrawable, ISerialisableDrawable
    {
        ...
        /// <summary>
        /// Current value in range [0, 1]. Updated from the static Pill bindable.
        /// </summary>
        ...
        protected override void LoadComplete()
        {
            ...
            // Bind the Pill count (int) to our Current (double in 0..1).
            ManiaModO2Judgement.Pill.BindValueChanged(pill =>
            {
                if (IsDisposed) return;
                // Map pill count to 0..1. Assume max pill count is 5.
                Current.Value = Math.Clamp(pill.NewValue / 5.0, 0, 1);
            }, true);
        }
        ...
        protected virtual void Flash() => marker.Flash(Current.Value >= epic_cutoff);
